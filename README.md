...
- 达到掌握标准后会提示“可以结束回顾”，但用户仍可继续刷题、追问或切主观题
...
系统会先为当前知识点拆出一组具体内容方面，再按内容方面分批生成题库。用户作答时不再每题等待模型临时出题，而是从已生成题库中按顺序取题。只要客观题数量、正确率和内容方面覆盖达标，即使不做简答题，也会被提示已达到掌握标准；这个提示是建议性状态，不会强制终止回顾。
...
- `mastered` 表示系统根据题量、覆盖、正确率和最近表现判断已达到掌握标准。
- `completed` 表示用户主动结束本轮回顾。
- 达到掌握标准后，用户仍可继续下一题、换题、追问或切换主观题；只有点击“结束回顾”才会关闭本轮。

当用户从“最近回顾”选择“重点回顾”时，系统会先把历史错题、低分内容方面、focusSubpoint、错误点、遗漏点和判断策略按内容方面与子点分组抽样，再由模型归纳真正的薄弱概念、优先级、targetSubpoints、misconceptions 和出题策略，最后据此重新生成重点题库。即使当前正在回顾另一个知识点，点击左侧“重点回顾”也会立即切换到目标知识点，并防止旧会话中的异步请求覆盖新界面。
